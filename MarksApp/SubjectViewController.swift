...
class SubjectViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //
        let index = self.subjectModel.subjectsAcronyms.firstIndex(of: self.acronymLabel.text!)
        self.subject = self.subjectModel.availableSubjects[index!]
        let matriculas = self.subject.matriculas
        if let _ = matriculas?.contains(self.userAccount) {
            //Está matriculado
        ...
        //self.subjectModel.matriculasLength(asignatura: self.acronymLabel.text!, account: self.userAccount!)
        /*
        self.subjectModel.call(asignatura: self.acronymLabel.text!, atributo: "matriculas")
        Timer.scheduledTimer(withTimeInterval: 0.2, repeats: true) { timer in
            if self.subjectModel.subjectLoaded {
                timer.invalidate()
            }
        }
        let index = self.subjectModel.subjectsAcronyms.firstIndex(of: self.acronymLabel.text!)
        self.subject = self.subjectModel.availableSubjects[index!]
        if let contains = self.subject.matriculas?.contains(self.userAccount) {
            if contains {
                self.notRegisteredButton.isHidden = true
                self.registeredLabel.isHidden = false
         // Consultar las notas
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.isRegistered {
            ...
            if let nota = calificaciones?[self.userAccount]?[indexPath.row] {
    ...
    @IBAction func register(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: "Matricularme", style: .default, handler: { action in
            ...
            if let name = alert.textFields?.first?.text, let email = alert.textFields?.last?.text {
                ...
                print("Llamo a setMatricula")
                // Llamar a send setMatricula(nombre, email, account)
            }
        }))
